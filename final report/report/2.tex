...
本节按方法部分假设提出的顺序报告了四个阶段的结果。阶段 1 和 2 检验了 PCA 投影的 OES 特征和超参数调优是否足够；这两个假设均被否定。阶段 3 检验了基于物理原理的 OES 特征是否能够揭示预测信号，阶段 4 则确定了去除冗余后剩余的最小特征子集。主要发现是阶段 1-2 和阶段 3 之间的显著变化，以及阶段 4 中对精简模型的统计验证。


The failure is structural rather than an artifact of improper dimensional reduction: the 11 principal components retained already preserve $\geq\!95\%$ of the spectral variance




两种独立的降维策略均能解决冗余问题，且结果一致。以岭射电为例，分类消融（表6，上半部分）在保留放电参数的同时，去除了不同的OES特征类型：去除除三个光谱比值以外的所有特征后，R² = 0.906；去除除三个波段积分以外的所有特征后，R² = 0.905。两者均与仅包含放电特征的配置B（0.904）相匹配，并显著优于包含全部13个特征的配置（0.798）。相比之下，单波长强度表现最差（0.823），这与其对绝对强度漂移的敏感性相符。反向消除轨迹（表6，下半部分）也呈现出相同的趋势：R²值从包含13个OES特征时的0.798上升到仅包含一个OES特征（CO₂⁺波段积分）时的峰值0.918，然后在不包含任何OES特征时回落至0.904，这证实了OES除了放电参数外，还提供了更多信息，但这些信息仅通过少量非冗余通道获得。

两种策略得到的峰值模型虽然接近但不完全相同：反向消除倾向于包含5个特征的模型（1个OES波段+4个放电特征，R²=0.918），而类别消融则保留了所有三个比值（3个比值+4个放电特征，共7个特征）。在对包含7个特征的模型集重新拟合岭回归模型后，R²值从0.906（类别消融结果）提升至0.920。对该重新拟合的模型进行2000次置换标签重排检验，未发现R²值与观测值相当的零假设，排除了过拟合的可能性。推荐使用包含7个特征的模型，因为它保留了所有三种比值类型——OH/Hα、O/OH和Hα/Hβ——这些比值类型本身具有漂移不变性且具有物理意义，而反向消除法得到的更简单的模型则依赖于单个波段积分。
...
图 7 展示了在第一阶段至第四阶段探索的所有模型-配置组合的总体排名中，经过修剪的 7 特征岭模型的位置。该修剪模型位居榜首，领先于经过调优的神经网络，并且远超所有基于 PCA 的配置。至关重要的是，必须结合自举重叠情况来解读此排名：排名前几的模型之间的绝对顺序在噪声范围内，但顶级模型类别（基于少量物理特征的简单线性回归器）与在第一阶段至第二阶段占据主导地位的基于 PCA 的配置之间存在显著差异。
