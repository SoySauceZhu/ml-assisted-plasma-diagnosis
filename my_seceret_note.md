...
总体可以说。在小数据里，我使用ridge这种L1, Lasso这种L2的简单线性回归方式，在config B 获得的效果很好。但是加上OES的config C下，效果反而不好了。这个时候，我们使用了RF的回归以及MLP，CNN这两个深度学习模型训练config C的模型，效果有所提升。但是仍然没有config B下线性回归的效果好。后来我们使用外部数据集进行特征提取，也尝试使用领域知识进行特征提取，效果………………。
