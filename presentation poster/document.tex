...
\begin{frame}{Phase 1: Results --- R$^2$ Heatmap (7 Models $\times$ 3 Configs)}
    \begin{figure}
        \centering
        \includegraphics[width=0.75\textwidth]{image/heatmap_R2.png}
    \end{figure}
    \begin{itemize}
        \item \textbf{Best:} Ridge Config B achieved R$^2$ = 0.90 (discharge params only)
        \item OES-based configs (A, C) performed poorly with blind PCA features
    \end{itemize}
\end{frame}
...
\begin{frame}{Phase 2: Bayesian Optimization}
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{@{}llccc@{}}
            ...
            \midrule
            MLP & C & $-1.13$ & 0.37 & $+1.50$ \\
            MLP & A & $-0.85$ & 0.37 & $+1.22$ \\
            RF  & B & 0.38 & 0.75 & $+0.37$ \\
            MLP & B & 0.57 & 0.86 & $+0.29$ \\
            CNN & C & 0.69 & 0.77 & $+0.09$ \\
            \bottomrule
        \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}{Phase 2: Results --- Phase 1 vs Phase 2 Comparison}
    \begin{figure}
        \centering
        \includegraphics[width=0.85\textwidth]{image/phase1_vs_phase2_comparison.png}
    \end{figure}
    \begin{itemize}
        \item Tuning universally beneficial, but Config B still dominant
        \item OES features via PCA remained weak even with optimized models
    \end{itemize}
\end{frame}
...
\begin{frame}{Phase 3: 13 Physics-Based OES Features}
    ...
    \begin{tabular}{@{}cllll@{}}
        ...
        F4  & I(486\,nm)        & Intensity & H$\beta$  & Electron diagnostics \\
        F5  & I(337\,nm)        & Intensity & N$_2$ SPS & High-energy $e^-$ marker \\
        F6  & I(406\,nm)        & Intensity & CO$_2^+$  & CO$_2$ ionization degree \\
        F7  & I(516\,nm)        & Intensity & C$_2$     & Deep decomposition \\
        \midrule
        ...
        \midrule
        F11 & I(309)/I(656)     & Ratio & OH/H$\alpha$           & OH availability \\
        F12 & I(777)/I(309)     & Ratio & O/OH                   & Radical pool balance \\
        F13 & I(656)/I(486)     & Ratio & H$\alpha$/H$\beta$     & Balmer decrement \\
        \bottomrule
    \end{tabular}
    \end{adjustbox}
\end{frame}
...
%% ---------------------------------------------------------------------------
\begin{frame}{Phase 3: Results --- Cross-Phase Comparison}
    \begin{figure}
        \centering
        \includegraphics[width=0.85\textwidth]{image/phase1_vs_phase3_comparison.png}
    \end{figure}
    \begin{itemize}
        \item \textbf{Ridge Config C:} R$^2$ improved from $-0.17$ to $0.80$ ($+0.97$)
        \item \textbf{MLP Config C:} R$^2$ improved from $-1.13$ to $0.82$ ($+1.95$)
        \item Domain knowledge dramatically unlocked OES predictive power
    \end{itemize}
\end{frame}
...
\begin{frame}{Phase 4: Multi-Model Feature Importance Consensus}
    \begin{figure}
        \centering
        \includegraphics[width=0.85\textwidth]{image/feature_importance_heatmap.pdf}
    \end{figure}
    \begin{itemize}
        \item Ranked 17 features across Ridge, PLS, RF, and MLP (SHAP)
        \item \textbf{Top features:} flow rate, CO$_2^+$ band integral, pulse width
    \end{itemize}
\end{frame}
